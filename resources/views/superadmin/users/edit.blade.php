...
@section('title', 'Modifier ' . $user->name . ' - BloodLink')
@section('description', 'Modifier les informations de l\'administrateur ' . $user->name)
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Modifier {{ $user->name }}</h1>
            <p class="mt-2 text-gray-600">Modifier les informations de l'administrateur</p>
        </div>
    ...
    <div class="bg-white shadow-md rounded-lg overflow-hidden">
        <form method="POST" action="{{ route('superadmin.users.update', $user) }}" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    ...
                    <div>
                        ...
                        <select name="role"
                            ...
                            <option value="admin_banque" {{ old('role', $user->role) === 'admin_banque' ? 'selected' : '' }}>Administrateur de Banque</option>
                        </select>
                        @error('role')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
